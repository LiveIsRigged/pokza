...
type Phase = 'context' | 'holeCards' | 'street-preflop' | 'street-flop' | 'street-turn' | 'street-river' | 'review';
...
export function LiveHandCreator({ onCreated, onCancel }: LiveHandCreatorProps) {
  ...
  const pushSnapshotAndGo = (nextPhase: Phase, patch: Partial<Omit<Snapshot, 'phase'>> = {}) => {
    setHistory((h) => [...h, { phase, context, seats, heroCards, actions, activeSeatIds, board }]);
  ...
  const finalize = (finalActions: Action[], finalBoard: Board) => {
    const seatsWithCards = seats.map((s) => (s.isHero ? { ...s, holeCards: heroCards as [Card, Card] } : s));
    const hand: Hand = {
      id: `hand-${Date.now()}`,
      variant: 'nlhe',
      ...
      return (
        <StreetStep
          ...
          onHandEndsEarly={(_board, newActions, remaining) => {
            pushSnapshotAndGo('review', { actions: [...actions, ...newActions], activeSeatIds: remaining, board: {} });
          }}
        />
      );
    }
